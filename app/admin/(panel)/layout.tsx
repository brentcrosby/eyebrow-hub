...
function AdminSideNav({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <aside className="flex h-full w-72 shrink-0 flex-col bg-[#FFFAF4] px-8 py-8 shadow-[2px_0_12px_rgba(167,140,122,0.15)] sm:w-80">
...
export default function AdminPanelLayout({
  ...
  // state to synchronise afterwards.
  useEffect(() => {
    if (!isNavOpen) return;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f5f1eb]">
      {/* Fixed 320px of sidebar left roughly 55px of content at 375px, inside an
      ...
      {/* min-w-0 lets this column shrink below its content width; without it a
          wide child would push the layout out again. */}
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex shrink-0 items-center gap-3 border-b border-[#eadfce] bg-[#FFFAF4] px-4 py-3 lg:hidden">
          <button
            type="button"
            onClick={() => setIsNavOpen(true)}
            aria-label="Open navigation"
            aria-expanded={isNavOpen}
            className="rounded-lg border border-[#eadfce] p-2 text-[#7a5a3c]"
          >
            <svg
              width="20"
              height="20"
